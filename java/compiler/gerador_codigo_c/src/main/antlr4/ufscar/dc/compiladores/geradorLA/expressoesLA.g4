...
cmdSe:
      ('se' expressao 'entao' cmdEntao+=cmd* ('senao' cmdSenao+=cmd*)? 'fim_se');
...
numero_intervalo:
      (opUn1=op_unario? NUM_INT ('..' opUn2=op_unario? NUM_INT)?);
...
parcela_unario:
      (('^'? identificador)                                     |
       (IDENT '(' expId+=expressao (',' expId+=expressao)* ')') |
       NUM_INT                                                  |
       NUM_REAL                                                 |
       ('(' expAv=expressao ')')                                );
