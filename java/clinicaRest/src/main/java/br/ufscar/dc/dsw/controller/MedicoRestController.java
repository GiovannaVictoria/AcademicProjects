...
public class MedicoRestController {
	...
	private void parse(Medico medico, JSONObject json) {
		Map<String, Object> map = (Map<String, Object>) json.get("medico");
		...
		medico.setUsername((String) map.get("username"));
		medico.setPassword(encoder.encode((String) map.get("password")));
		medico.setName((String) map.get("name"));
		medico.setEmail((String) map.get("email"));
		medico.setRole((String) map.get("role"));
		medico.setEnabled((boolean) map.get("enabled"));
		medico.setCRM((String) map.get("CRM"));
		medico.setEspecialidade((String) map.get("especialidade"));
	}
	...
	public ResponseEntity<List<Medico>> lista(@PathVariable("nome") String nome) {
		List<Medico> lista = service.buscarPorEspecialidade(nome);
		if (lista == null) {
			return ResponseEntity.notFound().build();
		}
		return ResponseEntity.ok(lista);
